...
function loadWinningTicketsTable() {
    ...
    for(let i in winningTickets){
        //this is a helper function
        buildTicketRow(tableBody, winningTickets[i]);
    }
}
function buildTicketRow(tbody, theTicket){
    let newRow = tbody.insertRow(-1);
    let tableData1 = newRow.insertCell(0);
    let tableData2 = newRow.insertCell(1);
    let tableData3 = newRow.insertCell(2);
    tableData1.innerHTML = theTicket.tixNum;
    tableData2.innerHTML = `$ ${(theTicket.prize).toFixed(2)}`;
    tableData3.innerHTML = theTicket.expires;
